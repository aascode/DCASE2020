...
from network_att import model_resnet
from utils_att import LR_WarmRestart, MixupGenerator, ValGenerator
...
model_path = 'models/DCASE_1a_Task_development_9.h5'
channels = '7ch'
is_skip = True
...
if is_skip is False:
    classes = ['airport','bus','metro_station','metro','park','public_square','shopping_mall','street_pedestrian','street_traffic','tram']
    class_list = []
...
model = model_resnet(NumClasses,
                     input_shape=[NumFreqBins,NumFrames,7*num_audio_channels], 
                     num_filters=24,
                     wd=1e-3)
...
ValsDataGen = ValGenerator(LM_val, 
                           y_val, 
                           batch_size=batch_size,
                           crop_length=crop_length)()

history = model.fit_generator(TrainDataGen,
                              validation_data=ValsDataGen,
                              epochs=num_epochs, 
                              verbose=1, 
                              workers=8,
                              max_queue_size=100,
                              ...
                              validation_steps=np.ceil(LM_val.shape[0]/batch_size)) 
