...
def pad(X):
    ...
    if X_len >= max_len:
        ...
        X_repeat = np.repeat(X,num_repeats, axis=2)
...
class ValGenerator():
    def __init__(self, X_val, y_val, batch_size=32, crop_length=400):
        self.X_val = X_val
        self.y_val = y_val
        self.batch_size = batch_size
    ...
    def __data_generation(self, batch_ids):
        ...
        X = self.X_val[batch_ids[:self.batch_size]]
        X = pad(X)
        
        if isinstance(self.y_val, list):
            y = []
            for y_val_ in self.y_val:
                tmp = y_val_[batch_ids[:self.batch_size]]
                y.append(tmp)
        else:
            y = self.y_val[batch_ids[:self.batch_size]]
