...
from keras.layers import AveragePooling2D, Input, concatenate, Lambda, Reshape, Dropout
...
def model_resnet(num_classes,input_shape=[128,423,7],num_filters=24,wd=1e-3):
    My_wd = wd
    num_res_blocks=2
    ...
    OutputPath = Lambda(lambda x: K.reshape(x, (8, 1, 128*27, 10)))(OutputPath)
    OutputPath = Lambda(lambda x: K.squeeze(x, 1))(OutputPath)
    OutputPath = AttentionLayer(name='attention')(OutputPath)
    #OutputPath = Dropout(0.5)(OutputPath)
